pipeline {
    ...
    environment {
        ...
        REMOTE_HOST = "13.125.112.169"          // 원격 (spring) 서버 탄력적 IP 본인 값으로 수정
        
        REMOTE_DIR = "/home/ec2-user/deploy"    // 원격 서버 배포 경로
        SSH_CREDENTIALS_ID = "5ffccf60-c196-4f62-a76f-4abefccd713a" //  본인 Credentials ID로 수정
    }

    stages { // stages : 실제 수행하는 단계 정의
